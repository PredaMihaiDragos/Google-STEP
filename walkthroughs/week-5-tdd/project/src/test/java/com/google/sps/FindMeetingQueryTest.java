...
public final class FindMeetingQueryTest {
  ...
  public void optionalNotInvited() {
    // Test based on everyAttendeeIsConsidered
    // Add one optional attendee with an all day event
    //
    // Events  :       |--A--|     |--B--|
    //           |--------------C--------------|
    // Day     : |-----------------------------|
  ...
  public void optionalInMiddle() {
    // Test based on everyAttendeeIsConsidered
    // Add optional attendee C between A and B
    //
    // Events  :       |--A--|--C--|--B--|
    // Day     : |-----------------------------|
    // Options : |--1--|                  |--2--|
  ...
  @Test
  public void oneOptionalInJustEnoughRoom() {
    // Test based on justEnoughRoom
    // Add optional attendee B somewhere in the room
    // It should be ignored because it makes the available time smaller than the meeting time
    //
    // Events  : |--A--|  |--B--|----A----| 
    // Day     : |------------------------|
    // Options :       |--------|
  ...
  public void onlyOptionals() {
    // Only 2 optional attendees with several gaps in their schedules
    //
    // Events  : |--A--| |--B--|    |--B--|    |--A--|
    // Day     : |-----------------------------------|
    // Options :               |----|     |----|
    ...
    Collection<Event> events = Arrays.asList(
        ...
        new Event("Event 3", TimeRange.fromStartEnd(TIME_1100AM, TIME_1200AM, false),
            Arrays.asList(PERSON_B)),
        new Event("Event 4", TimeRange.fromStartEnd(TIME_1400AM, TimeRange.END_OF_DAY, false),
            Arrays.asList(PERSON_A)));
    ...
    Collection<TimeRange> actual = query.query(events, request);
    Collection<TimeRange> expected =
        Arrays.asList(TimeRange.fromStartEnd(TIME_0930AM, TIME_1100AM, false),
                      TimeRange.fromStartEnd(TIME_1200AM, TIME_1400AM, false));
  ...
  public void onlyOptionalsNoHaps() {
    ...
    Collection<Event> events = Arrays.asList(
        ...
        new Event("Event 2", TimeRange.fromStartEnd(TIME_0900AM, TIME_0930AM, false),
            Arrays.asList(PERSON_B)),
        new Event("Event 3", TimeRange.fromStartEnd(TIME_1000AM, TIME_1200AM, false),
            Arrays.asList(PERSON_B)),
        new Event("Event 4", TimeRange.fromStartEnd(TIME_1200AM, TimeRange.END_OF_DAY, false),
            Arrays.asList(PERSON_A)));
